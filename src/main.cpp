...
void setup() {
 ...
 // пишемо в память lcd символи
  ...
  pinMode(RELAY6_PIN, OUTPUT); // порт для мотора-редуктора
...
if (flag_Startincubation==0)//інкубатор не запущено в роботу, а лише включили панель управління і здійснюється нагрів до температури яйця! (для уникнення конденсату)
...
// добавити блок відправки тривожних повідомлень
 if(flag_power) // якщо піднято флаг мережі 220 вольт
 {
  ton();
  int power = 
